...
if ( !class_exists( 'G5_Terms_List_Table' ) ) :

class G5_Terms_List_Table extends WP_List_Table {
	...
	public function display_rows_or_placeholder() {
		...
		$terms = g5_get_terms( $taxonomy, $args );
	...
	private function _rows( $taxonomy, $terms, &$children, $start, $per_page, &$count, $parent = 0, $level = 0 ) {
		...
		foreach ( $terms as $key => $term ) {
			...
			if ( $count == $start && $term->parent > 0 && empty( $_REQUEST['s'] ) ) {
				...
				$p = $term->parent;
				while ( $p ) {
					$my_parent = g5_get_term( $p, $taxonomy );
	...
	public function column_name( $tag ) {
        ...
        $g5_config = G5_var::getInstance()->get_options('config');
		$taxonomy = $this->screen->taxonomy;
		$tax = get_taxonomy( $taxonomy );
		...
		$qe_data = g5_get_term( $tag->term_id, $taxonomy, OBJECT, 'edit' );
		$edit_link = esc_url( g5_get_edit_term_link( $tag->term_id, $taxonomy, $this->screen->post_type, '', $this->screen->bo_table ) );
        $delete_link = g5_get_edit_term_link( $tag->term_id, $taxonomy, $this->screen->post_type, '', $this->screen->bo_table , 'delete' );
		...
		if ( current_user_can( $tax->cap->delete_terms ) && $tag->term_id != $default_term )
			$actions['delete'] = "<a class='g5_delete-tag' href='" . wp_nonce_url( $delete_link, 'g5_delete-tag_' . $tag->term_id ) . "'>" . __( 'Delete' ) . "</a>";
		if ( $tax->public && $g5_config['cf_include_search_tag'] )
...
?>
	...
	<?php
		...
		$core_columns = array( 'cb' => true, 'description' => false, 'name' => true, 'slug' => true, 'posts' => true );
