...
if (isset($_POST['ca_name'])) {
    $ca_name = substr(trim($_POST['ca_name']),0,255);
}
...
if (isset($_POST['wp_html']) && $_POST['wp_html'] && $html ) {
    $wp_html = $_POST['wp_html'];
    // 워드 프레스 에디터인 경우
    $wr_content = wp_unslash($wr_content);
}
...
if ($w == '' || $w == 'r') {
    if ($member['user_id']) {
        ...
        $user_name = g5_clean_xss_tags(trim($_POST['user_name']));
        if (!$user_name)
            g5_alert('이름은 필히 입력하셔야 합니다.');
        $user_pass = g5_sql_password(trim($_POST['user_pass']));
    ...
    // insert
    $result = $wpdb->insert($write_table, $g5_data);
    ...
    // bo_table 에 게시글 1 증가
    g5_sql_query("update {$g5['board_table']} set bo_count_write = bo_count_write + 1 where bo_table = '{$bo_table}'");
    ...
    $result = $wpdb->update($write_table, $g5_data, $where);
...
$result = $wpdb->update($write_table, $after_update, $where);
